...
interface Target {
  player: GamePlayer;
  /** Slug of the full name, e.g. "aaron-judge". */
  slug: string;
  /** Slug of the surname only, e.g. "judge". */
  surname: string;
}
...
/** Last whitespace-separated token of a name, slugified. */
function surnameSlug(name: string): string {
  const parts = name.trim().split(/\s+/);
  return slugify(parts[parts.length - 1] ?? name);
}
...
export default function BeatTheClock() {
  ...
  // Load the dataset and compute the top-30 home-run hitters once.
  useEffect(() => {
    let alive = true;
    loadGamesData()
      .then((data) => {
        ...
        const top = data.players
          ...
          .map<Target>((player) => ({
            ...
            surname: surnameSlug(player.name),
          }));
        setTargets(top);
  ...
  const finish = useCallback(
    (finalFound: Set<number>) => {
      ...
      recordScore(GAME, n, true);
      setBest((b) => Math.max(b, n));
      void submitScore(GAME, n, true);
      // A clean sweep earns confetti + a fanfare; a solid run just settles.
      if (n >= TARGET_COUNT) {
        setCelebrate(true);
        fanfare();
      } else {
  ...
  const submitGuess = useCallback(() => {
    ...
    const idx = targets.findIndex((t, i) => {
      ...
      // Exact full name or exact surname.
      if (t.slug === guess || t.surname === guess) return true;
      // Fuzzy: a 3+ char fragment that appears in the full name.
    ...
    <div className="card" style={styles.card}>
      ...
      <header style={styles.header}>
        <div>
          ...
          <p style={styles.sub}>Name the top {TARGET_COUNT} home-run hitters</p>
        </div>
        ...
        <div style={styles.startWrap}>
          <ul style={styles.rules}>
            ...
            <li>
              ...
              <strong>top {TARGET_COUNT} home-run hitters</strong> in the dataset.
            </li>
            <li>Hit Enter after each name. Surnames usually work.</li>
            <li>Name as many sluggers as you can before the clock runs out.</li>
          </ul>
      ...
      {phase === "playing" ? (
        <div style={styles.inputWrap}>
          <input
            ...
            onKeyDown={onKeyDown}
            placeholder="Type a slugger…"
            aria-label="Name a home-run hitter"
            autoComplete="off"
            autoCorrect="off"
            autoCapitalize="off"
            spellCheck={false}
        ...
        {slots.map((t, i) => {
          ...
          const tone: "found" | "missed" | "locked" = isFound
            ...
            : phase === "over"
              ? "missed"
              : "locked";
          const [primary] = teamColors(t.player.team);
          return (
              ...
              {revealed ? (
                <>
                  ...
                  <span style={styles.slotMeta}>
                    <span
                      style={{ ...styles.teamDot, background: primary }}
                      aria-hidden="true"
                    />
                    {t.player.team} · {t.player.hr} HR
                  </span>
                </>
              ) : (
