...
function buildRound(pool: GamePlayer[], rand: () => number): Round | null {
  ...
  // Distractors must share the same kind (hitter vs pitcher) so the stat line
  // doesn't give the answer away. Prefer the same position, then same kind.
  const sameKind = pool.filter((p) => p.id !== target.id && p.kind === target.kind);
  if (sameKind.length < 3) return null;

  const samePos = sameKind.filter((p) => p.pos === target.pos);
  const otherPos = sameKind.filter((p) => p.pos !== target.pos);

  const distractors: GamePlayer[] = [];
  const seen = new Set<number>([target.id]);

  const take = (candidates: GamePlayer[]) => {
    for (const p of pickN(candidates, candidates.length, rand)) {
  ...
  take(samePos);
  if (distractors.length < 3) take(otherPos);
...
export default function CareerPath() {
  ...
  const endRun = useCallback((finalScore: number) => {
    recordScore("career-path", finalScore, true);
    void submitScore("career-path", finalScore, true);
    setBest(getScore("career-path").best);
    setPhase("over");
  }, []);
  ...
  const dotColor = target ? teamColors(target.team)[0] : "var(--muted)";
...
const styles: Record<string, React.CSSProperties> = {
  ...
  team: { fontWeight: 700, fontSize: 16 },
