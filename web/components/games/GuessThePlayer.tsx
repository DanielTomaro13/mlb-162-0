...
/** A closer/reliever whose calling card is saves rather than strikeouts. */
function isSaveArtist(p: GamePlayer): boolean {
  return p.kind === "pit" && (p.pos === "CL" || p.pos === "RP") && p.sv >= 100;
}

/**
 * Seven clues, ordered hardest → easiest. The stat clues are kind-aware:
 * hitters show power/run-production lines, pitchers show strikeout/win/ERA
 * lines (closers lead with saves).
 */
function buildClues(p: GamePlayer): string[] {
  const kindLabel = p.kind === "pit" ? "A Pitcher." : "A Hitter.";

  const headline =
    p.kind === "pit"
      ? isSaveArtist(p)
        ? `${p.sv} career saves`
        : `${p.so} career strikeouts`
      : `${p.hr} career home runs`;

  const secondary =
    p.kind === "pit"
      ? `${p.w} wins, ${p.eraAvg.toFixed(2)} ERA`
      : `${p.rbi} RBI, ${p.sb} SB`;
...
export default function GuessThePlayer() {
  ...
  function submitGuess(e: React.FormEvent) {
    ...
    if (isMatch(value)) {
      ...
      submitScore(GAME, score, true);
      return;
    }

    // Wrong guess: penalise and reveal the next clue.
    const next = Math.max(0, score - WRONG_COST);
    setScore(next);
    setGuess("");
  ...
  const shareText = `MLB 162-0 Guess the Player #${dayNumber()} — ${
    status === "won" ? `${score} pts` : "missed"
  }\nmlb162-0.com/games/guess-the-player`;
      ...
      >
        <div>
          ...
          <div style={{ fontSize: ".82rem", color: "var(--muted)", marginTop: 2 }}>
            {finished ? "Done for today" : `Clue ${revealed} of ${TOTAL_CLUES}`}
          </div>
        </div>
